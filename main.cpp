...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < model.nfaces(); i++)
    {
        ...
        {
            Vec3f v = model.vertex(face[vId]);
            screenCoords[vId] =
                Vec2i((v.x + 1.0) * width / 2.0, (v.y + 1.0) * height / 2.0);
            worldCoord[vId] = v;
        }

        Vec3f faceNormal = (worldCoord[2] - worldCoord[0] ^ (worldCoord[1] - worldCoord[0]));
        faceNormal.normalize();
        float intensity = faceNormal * lightdir;
        if (intensity > 0)
